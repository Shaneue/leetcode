...
public class DefuseTheBomb {
    public int[] decrypt(int[] code, int k) {
        ...
        if (k == 0) {
            ...
            for (int i = code.length; i < nums.length; i++) {
                int sum = 0;
                for (int j = i - Math.abs(k); j < i; j++) sum += nums[j];
                ret[i - code.length] = sum;
            }
        } else {
            for (int i = 0; i < code.length; i++) {
                int sum = 0;
                for (int j = i + 1; j <= i + k; j++) sum += nums[j];
                ret[i] = sum;
            }
        }
        return ret;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(d.decrypt(new int[]{5, 7, 1, 4}, 3)));
    }
}
