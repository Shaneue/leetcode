...
public class StockPriceFluctuation {
    ...
    public void update(int timestamp, int price) {
        if (timeMap.containsKey(timestamp)) {
            int priceBefore = timeMap.get(timestamp);
            Set<Integer> set = map.get(priceBefore);
            set.remove(timestamp);
            if (set.size() == 0) map.remove(priceBefore);
        }
        timeMap.put(timestamp, price);
        Set<Integer> set = map.getOrDefault(price, new HashSet<>());
        set.add(timestamp);
        map.put(price, set);
